...
\title{The fundamental group of the Stringy Kähler Moduli Space acts on \\ $D^b(X)$ via spherical twists}
...
We would like to construct an algebraic variety that parametrises the actions quotients. \textbf{Geometric Invariant Theory} (GIT) is the theory that tells us what to throw away before we quotient so that we get a good/geometric quotient. \cite{mumford1994geometric}
\begin{example}
  Consider $\mathbb{C}^*$ acting on $\mathbb{C}^2$ linearly, i.e. $\lambda \cdot (z_1, \, z_2) = (\lambda z_1,\, \lambda z_2)$.
  \begin{center}\includegraphics{orbits of P1.png}\end{center}
  \begin{picture}(0,0)
  \put(435,295){$aL$} 
  \put(550,80){$a$}
  ...
  \end{picture} 
\end{example}
In general GIT quotients are not unique, and depending on a choice of \textit{stability} 
...
\Head{Geometric Invariant Theory and Wall Crossing}
\sf
-first explain action
-describe the wall crossing algorithm
-add def of calabi yau
